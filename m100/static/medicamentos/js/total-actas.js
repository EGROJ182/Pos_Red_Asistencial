document.addEventListener('DOMContentLoaded', function () {
    const btnOcultarActas = document.querySelector('.button-actas');
    ...
    if (btnOcultarActas) {
        btnOcultarActas.addEventListener('click', function () {
            ...
            if (isHidden) {
                ...
                animateButtonText(btnOcultarActas, 'Ver Actas');
            } else {
                actasSection.style.maxHeight = actasSection.scrollHeight + "px"; // Expande la sección
                animateButtonText(btnOcultarActas, 'Ocultar Actas');
            }
        });
    } else {
    ...
    // Verifica si el botón existe antes de añadir el listener
    if (!actasSection) {
        console.error('La sección de actas no se encontró en el DOM.');
    }
});
