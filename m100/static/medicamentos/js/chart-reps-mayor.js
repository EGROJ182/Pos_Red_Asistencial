document.addEventListener('DOMContentLoaded', function () {
  ...
  // 6. Convertir a un array y ordenar de mayor a menor
  ...
  const ctx = document.getElementById('chartTopRepsMayor');
  if (!ctx) {
    console.warn("No se encontró <canvas id='chartTopRepsMayor'> en la plantilla.");
    return;
  }
  ...
  const myBarChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Top 5 Departamentos con Más Reps',
          font: { size: 16 }
        },
        tooltip: {
          callbacks: {
