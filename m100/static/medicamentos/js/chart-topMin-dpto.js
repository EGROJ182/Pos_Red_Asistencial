document.addEventListener('DOMContentLoaded', function () {
  ...
  try {
    ...
    // console.log("Datos del chart data:", chartData); // Verificar datos
  } catch (error) {
  ...
  const ctx = document.getElementById('chartTopMenorDpto');
  if (!ctx) {
    console.warn("No se encontró <canvas id='chartTopMenorDpto'> en la plantilla.");
    return;
  }
  ...
  const myBarChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Top 5 Departamentos con menor cantidad de Prestadores',
          font: { size: 16 }
        },
        tooltip: {
          callbacks: {
