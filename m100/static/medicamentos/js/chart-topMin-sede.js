document.addEventListener('DOMContentLoaded', function () {
  ...
  const ctx = document.getElementById('chartTopSedeMenor');
  if (!ctx) {
    console.warn("No se encontró <canvas id='chartTopSedeMenor'> en la plantilla.");
    return;
  }
  ...
  const myBarChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        title: {
          display: true,
          text: 'Top 5 Departamentos con Menos Sedes',
          font: { size: 16 }
        },
        tooltip: {
          callbacks: {
